...
class WebSearchInstance(Mapping):
    def __init__(self, title='', year='', season='', episode='', quality='', source='', search_type='',
    ...
    def validate(self):
        if self.search_type is FileFlags.ANIME_DIRECTORY_FLAG:
            self.season = ''
            self.year = ''
            if len(self.episode) == 1:
                self.episode = '0' + str(self.episode)

        elif self.search_type is FileFlags.FILM_DIRECTORY_FLAG:
            self.season = ''
            self.episode = ''
            self.source = ''

        elif self.search_type is FileFlags.SHOW_DIRECTORY_FLAG:
            self.source = ''
            self.year = ''
            if len(self.episode) == 1:
                self.episode = '0' + str(self.episode)
