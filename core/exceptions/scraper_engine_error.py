...
class ScraperEngineNetworkError(Exception):
    """Raise when a webscrapers is unable to connect to a source"""
...
class ScraperEngineUnknownError(Exception):
    """Raise when a webscrapers is unable to connect to a source"""
    def __init__(self, webscraper_name, err, trace, *args):
        self.name = self.__class__.__name__
        self.trace = trace
        ...
        super(ScraperEngineUnknownError, self).__init__(self.message, err, webscraper_name, *args)
