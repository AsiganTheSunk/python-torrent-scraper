...
class RegexCommonExtension():
    ...
    def get_extension(self, stream, debug=False):
        '''
        This function retrieves the webscrapers of the file or directory from the stream using regular expresions
        :param stream: It represents the input string you're parsing
        :param debug: It represents the debug status of the function, default it's False
        :return: EXTENSION
        '''
        ...
        try:
            ...
            if debug:
                print('{0}: {1} :: webscrapers:{2}'.format(self.name, stream, extension))
            return extension
